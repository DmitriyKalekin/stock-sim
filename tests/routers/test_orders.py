...
def test_add_order__success(client):
    with client:
        ...
        # ------------- CREATE  BID -----------------
        original1 = {"player_id": player_id, "cls": "LIMIT", "type": "BID", "price": 36.60, "volume": 10.20}
        res = add_order(client, original1)
        assert res.status_code == 201
        assert res.json() == {"order_id": str(1).zfill(12)}
        # ------------- CREATE  ASK -----------------
        original2 = {"player_id": player_id, "cls": "LIMIT", "type": "ASK", "price": 36.60, "volume": 10.20}
...
def test_add_order__404_player_not_found__error(client):
    with client:
        # ------------- CREATE  BID -----------------
        original1 = {"player_id": "0001", "cls": "LIMIT", "type": "BID", "price": 36.60, "volume": 10.20}
        res = add_order(client, original1)
...
def test_add_order__negative_price_with_3_digits__error(client):
    with client:
        ...
        player_id = str(1).zfill(4)
        # ------ Add BID -------------
        original = {"player_id": player_id, "cls": "LIMIT", "type": "BID", "price": -0.123, "volume": 10.20}
...
def test_add_order__negative_volume_with_6_digits__error(client):
    with client:
        ...
        player_id = str(1).zfill(4)
        # ------ Add BID -------------
        original = {"player_id": player_id, "cls": "LIMIT", "type": "BID", "price": 36.60, "volume": -10.123456789}
        res = add_order(client, original)
        assert res.status_code == 422
        assert res.json() == {'detail': [
...
def test_add_order__wrong_type__error(client):
    with client:
        ...
        player_id = str(1).zfill(4)
        # ------ Add Order -------------
        original = {"player_id": player_id, "cls": "LIMIT", "type": "ABC", "price": 36.60, "volume": 10}
        res = add_order(client, original)
        assert res.status_code == 422
        assert res.json() == {
...
def test_get_order_by_id__success(client):
    with client:
        ...
        # ---------------- CREATE ASK FIRST -----------------
        original2 = {"player_id": player_id, "cls": "LIMIT", "type": "ASK", "price": 37.60, "volume": 10.20}
        res = add_order(client, original2)
        ...
        assert res.json() == {**json1_id, "ts": 0, **original1, "status": "ACTIVE"}
        # ---------------- AND READ SECOND -----------------
        res = client.get(f"/order/{order2_id}")
        assert res.status_code == 200
        assert res.json() == {**json2_id, "ts": 0, **original2, "status": "ACTIVE"}
...
def test_get_order_book__sort_bids_acs_asks_desc_success(client):
    with client:
        ...
        player_id = str(1).zfill(4)
        # ------------- CREATE  BID -----------------
        original1 = {"player_id": player_id, "cls": "LIMIT", "type": "BID", "price": 11.60, "volume": 11.20}
        wanted_id1 = {"order_id": str(1).zfill(12)}
        res = add_order(client, original1)
        assert res.status_code == 201
        assert res.json() == wanted_id1
        # ------------- CREATE  BID -----------------
        original2 = {"player_id": player_id, "cls": "LIMIT", "type": "BID", "price": 12.60, "volume": 12.20}
        wanted_id2 = {"order_id": str(2).zfill(12)}
        res = add_order(client, original2)
        assert res.status_code == 201
        assert res.json() == wanted_id2
        # ------------- CREATE  ASK -----------------
        original3 = {"player_id": player_id, "cls": "LIMIT", "type": "ASK", "price": 33.60, "volume": 33.20}
        wanted_id3 = {"order_id": str(3).zfill(12)}
        res = add_order(client, original3)
        assert res.status_code == 201
        assert res.json() == wanted_id3
        # ------------- CREATE  ASK -----------------
        original4 = {"player_id": player_id, "cls": "LIMIT", "type": "ASK", "price": 44.60, "volume": 44.20}
        ...
        assert res.json() == {
            ...
            "bids_cnt": 2,
            "asks_cnt": 2,
            "bids": [{**wanted_id2, "ts": 0, **original2, "status": "ACTIVE"}, {**wanted_id1, "ts": 0, **original1, "status": "ACTIVE"}],
            # special sort order!!!
            "asks": [{**wanted_id3, "ts": 0, **original3, "status": "ACTIVE"}, {**wanted_id4, "ts": 0, **original4, "status": "ACTIVE"}]
            # # special sort order!!!
        }
...
def test_delete_order_by_id__success(client):
    with client:
        ...
        # ------------- CREATE  ASK -----------------
        original2 = {"player_id": player_id, "cls": "LIMIT", "type": "ASK", "price": 12.60, "volume": 11.20}
